"""
ref: https://github.com/QwenLM/Qwen3-VL/blob/main/qwen-vl-utils/src/qwen_vl_utils/vision_process.py

add `video_resize_chunk_frames` in `fetch_video` reducing memory usage when loading long videos
"""
import base64
import copy
import logging
...
def fetch_video(ele: Dict[str, Any], image_patch_size: int = 14, return_video_sample_fps: bool = False,
                return_video_metadata: bool = False, video_resize_chunk_frames: int = None) -> Union[torch.Tensor, List[Image.Image]]:
    ...
    if video_resize_chunk_frames and video_resize_chunk_frames < nframes:
        resized_chunks = []
        for i in range(0, nframes, video_resize_chunk_frames):
            chunk = video[i:i + video_resize_chunk_frames]  # 切片，共享内存
            chunk_resized = transforms.functional.resize(
                chunk,
                [resized_height, resized_width],
                interpolation=InterpolationMode.BICUBIC,
                antialias=True,
            ).float()
            resized_chunks.append(chunk_resized)
...
def process_vision_info(
    ...
    video_resize_chunk_frames: int = None,
    ...
    for vision_info in vision_infos:
        if "image" in vision_info or "image_url" in vision_info:
            ...
            video_input, video_sample_fps = fetch_video(vision_info, return_video_sample_fps=True,
                        ...
                        video_resize_chunk_frames=video_resize_chunk_frames)
